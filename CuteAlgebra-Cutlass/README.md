...
*what*.
### 2.5 (cont'd) — Implicit tensors via counting/coordinate accessors

Explains something I'd been staring at without noticing: why Figure 3's
grids show bare numbers while Figure 1's grid showed real letters.

**The counting-iterator accessor `{a}` is a fake accessor — dereference is
a no-op.** Offset rule is normal (`{a}+b → {a+b}`), but
`*{a} → a` just hands back the number it's already holding — no real
memory fetch. So `T(c) = *(e+L(c))` collapses to `T(c) = a + L(c)`,
always. The tensor's "value" is just a number generated by the layout,
not real data.

**This is exactly what Figure 3's grids are plotting** — using `{0}`, so
`T(c)=L(c)` directly. Confirmed on my own layout, `a=100`:
`L=((2,2),2):((2,4),1)`, `c=((1,1),0)` (box g): `L(c)=6` →
`T(c)=100+6=106`. Figure 1 used a *real* accessor bound to actual memory
(`a`-`h`); Figure 3 uses this fake one — same `T=e∘L` formula, genuinely
different accessor, which is the whole reason one produces letters and
the other produces bare numbers.

**Same trick one level up for Figure 4a/b:** accessor `{(a,b)}` also has a
no-op dereference, just on coordinate pairs instead of single numbers —
`T(c)=L(c)` as a coordinate. Why Figure 4a's grid shows raw `(i,j)` pairs
per cell: it's literally the identity layout's output, echoed straight
through.
